...
class Sugerencia(models.Model):
    """
    Modelo para gestionar sugerencias y mejoras del sistema
    """
    TIPO_CHOICES = [
        ('PROCESO', 'Mejora de Procesos'),
        ('FUNCIONALIDAD', 'Nueva Funcionalidad'),
        ('INTERFAZ', 'Mejora de Interfaz'),
    ...
    # Usuarios
    solicitante = models.ForeignKey(Usuario, on_delete=models.CASCADE, related_name='sugerencias_solicitadas', verbose_name="Solicitante")
    asignado_a = models.ForeignKey(
        Usuario, 
        on_delete=models.SET_NULL, 
        null=True, 
        blank=True, 
        related_name='sugerencias_asignadas',
    ...
    def get_estadisticas(cls):
        ...
        en_revision = cls.objects.filter(estado='EN_REVISION').count()
        aprobadas = cls.objects.filter(estado='APROBADA').count()
        implementadas = cls.objects.filter(estado='IMPLEMENTADA').count()
    ...
    @classmethod
    def get_sugerencias_recientes(cls, limite=5):
        """Obtiene las sugerencias más recientes"""
        return cls.objects.all().order_by('-fecha_creacion')[:limite]
