#!/usr/bin/env python3
"""
Script para cargar TODOS los datos del backup completo en la base de datos online
...
def cargar_completo_online(backup_dir=None):
    """Cargar todos los datos del backup completo en la base de datos online"""
    
    # Si no se especifica directorio, buscar el más reciente
    if not backup_dir:
        backup_dirs = glob.glob("backups_completos_*")
        if not backup_dirs:
            print("❌ No se encontraron directorios de backup completos")
            print("💡 Ejecuta primero: python backup_completo_final.py")
            return False
    ...
    print(f"🚀 Cargando datos completos desde: {backup_dir}")
    print("=" * 60)
    ...
    json_files = [f for f in json_files if not f.endswith('RESUMEN_BACKUP_COMPLETO.json')]
    ...
    print()
    print("=" * 60)
    print("📋 RESUMEN DE CARGA COMPLETA")
    print("=" * 60)
...
def cargar_por_app(backup_dir=None, app_name=None):
    ...
    if not app_name:
        ...
        print("💡 Ejemplo: cargar_por_app(backup_dir, 'clientes')")
        return False
    
    print(f"🚀 Cargando datos de la app '{app_name}' desde: {backup_dir}")
    print("=" * 50)
    
    # Buscar archivos de la app específica
    pattern = os.path.join(backup_dir, f"*_{app_name}_*.json")
    json_files = glob.glob(pattern)
    
    if not json_files:
        print(f"❌ No se encontraron archivos para la app '{app_name}'")
        return False
    ...
    print(f"📊 Archivos encontrados para '{app_name}': {total_archivos}")
    print()
    ...
    print()
    print(f"🎉 App '{app_name}': {cargados}/{total_archivos} archivos cargados")
    return cargados == total_archivos

if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Cargar datos completos en la base de datos online')
    ...
    if args.app:
        # Cargar solo una app
        cargar_por_app(args.backup_dir, args.app)
    else:
        # Cargar todo
        cargar_completo_online(args.backup_dir) 